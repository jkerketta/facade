		...
		schedule[dateStr] = [
			{
				type: "reel",
				time: "07:00",
				description: "Morning transformation check-in and progress photo.",
			},
			{
				type: "story",
				time: "12:30",
				description: "Diet update with macros and meal prep tips.",
			},
			{
				type: "reel",
				time: "18:00",
				description: "Gym training highlights with form breakdown.",
			},
		];
	}
	return schedule;
		...
		for (let j = 0; j < numPosts; j++) {
			...
			schedule[dateStr].push({
				...
				description: type === "reel"
					? "Workout breakdown and transformation progress"
					: "Daily diet and nutrition check-in"
			});
		}
...
const InfluencerDetailsSidebar = ({
	...
	const maddie = isMaddie(influencer.name);
			...
			<div className='flex-1 flex flex-col items-center text-center'>
				...
				<div className='text-left w-full mt-6'>
					<h4 className='font-semibold text-sm mb-2 text-white/80'>
						...
						<p className='text-white/60 text-sm leading-relaxed line-clamp-4'>
							{maddie ? MADDIE_BACKGROUND : influencer.persona.background}
						</p>
					</div>
