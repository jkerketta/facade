...
const Step2AvatarGeneration: React.FC<Step2AvatarGenerationProps> = ({
	...
	const handleGenerate = async () => {
		...
		try {
			...
			if (!result?.image_url && !result?.path) {
				throw new Error(
					"Avatar API returned success but no image path was provided."
				);
			}
			const fullAvatarUrl =
				result.image_url ||
			...
			if (result.fallback) {
				setGenerationNotice(
					result.fallback_reason
						? `AI image service fallback was used: ${result.fallback_reason}`
						: "AI image service is currently limited, so we generated a fallback avatar."
				);
			}
		} catch (error) {
			...
			setGenerationNotice(`Avatar generation failed: ${message}`);
