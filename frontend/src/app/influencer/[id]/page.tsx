...
const InfluencerProfilePage = () => {
	...
	const loadLocalInfluencer = (influencerId: string): Influencer | null => {
		...
		try {
			...
			return {
				...
				persona: {
					background: match.persona?.background || "",
				...
				videos: match.videos || [],
				followers: match.followers || `${(Math.random() * 5).toFixed(1)}M`,
				engagement:
	...
	const fetchInfluencerData = useCallback(async () => {
		...
		try {
			const influencerRes = await fetch(
				`http://127.0.0.1:8000/influencer/${influencerId}`
			);
			if (!influencerRes.ok) {
			...
			const videoRes = await fetch(
				`http://127.0.0.1:8000/influencer/${influencerId}/videos`
			);
			let videos: Video[] = [];
			if (videoRes.ok) {
	...
	if (!influencer) {
		return (
			<div className='w-screen h-screen bg-[#111111] flex flex-col items-center justify-center text-white gap-4'>
				<p>{loadError || "Profile not found."}</p>
				<button
					onClick={() => router.push("/")}
					className='px-4 py-2 border border-white/30 hover:bg-white/10'
				>
					Back to Home
				</button>
			</div>
