...
async def generate_image(
    ...
    def generate_fallback_avatar(reason: str):
        storage_path = Path("storage/images")
        storage_path.mkdir(parents=True, exist_ok=True)

        size = (1024, 1024)
        img = Image.new("RGB", size, color=(20, 24, 35))
        draw = ImageDraw.Draw(img)

        # Soft layered circles for a simple but intentional fallback look.
        draw.ellipse((120, 120, 900, 900), fill=(38, 52, 77))
        draw.ellipse((220, 220, 800, 800), fill=(60, 88, 122))

        initials = "".join(
            [word[0].upper() for word in request.prompt.split()[:2] if word]
        ) or "AI"
        ...
        draw.text(
            ((size[0] - text_w) / 2, (size[1] - text_h) / 2 - 40),
            initials,
            fill=(235, 242, 255),
            font=font,
        )

        filename = f"generated_fallback_{uuid.uuid4().hex}.png"
        filepath = storage_path / filename
        img.save(filepath, format="PNG")
        compact_reason = (reason or "").replace("\n", " ").strip()
        if len(compact_reason) > 220:
            compact_reason = compact_reason[:220] + "..."
    ...
    if not api_key:
        return generate_fallback_avatar("Gemini API key not configured")

    client = genai.Client(api_key=api_key)

    max_retries = 3
    model_name = os.getenv("GEMINI_IMAGE_MODEL", "gemini-2.5-flash-image").strip()
    if not model_name:
    ...
    for attempt in range(max_retries):
        try:
            ...
            return generate_fallback_avatar(error_message)
            ...
            if attempt < max_retries - 1:
                ...
                return generate_fallback_avatar(
                    f"Image generation failed after {max_retries} attempts using {model_name}: {error_details}"
                )
